...
function File:get_name_with_extension()
    local path_split = coredor.string_split(self:path(), "/")
...
function File:get_name_with_out_extension()
    ...
    local name_with_extension_split =
        coredor.string_split(name_with_extension, ".")
    name_with_extension_split[#name_with_extension_split] = nil
    local name_with_out_extension =
        coredor.string_merge(name_with_extension_split, ".")
...
function File.list(path, glob)
    ...
    local files_pathes = coredor.string_split(files_as_text, "\n")
    local results = coredor.array_map(files_pathes, function(file_path)
